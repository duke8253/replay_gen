...
class RepalyFile:
    ...
    def random_populate(self, curr_trans_num, total_trans_num, url_dict, sess_lower, sess_upper, trans_lower, trans_upper, http_trans, tls_trans, h2_trans):
    ...
    def dump_to_disk(self, print_info):
        ...
        if print_info:
            print('Generated file {0}, with {1} sessions and {2} transactions.'.format(self.f_name, self.sess_count, self.trans_count))
...
def parse_args():
    ...
    parser.add_argument('-tl', '--trans-lower', dest='trans_lower', type=int, default=10, help='The lower limit of transactions per session.')
    parser.add_argument('-tu', '--trans-upper', dest='trans_upper', type=int, default=10, help='The upper limit of transactions per session.')
    parser.add_argument('-sl', '--sess-lower', dest='sess_lower', type=int, default=10, help='The lower limit of sessions per file.')
    parser.add_argument('-su', '--sess-upper', dest='sess_upper', type=int, default=10, help='The upper limit of sessions per file.')
    parser.add_argument('-tp', '--trans-protocols', dest='trans_protocols', type=str, default='all', help='A comma separated list of protocols that are allowed to be generated. Available options are: http, tls, h2, all.')
...
def main():
    ...
    while True:
        f_name = pathlib.PurePath(args.output).joinpath('{0}{1}.json'.format(args.prefix + '_' if args.prefix != '' else '', file_count))
